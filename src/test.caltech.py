...
for start, end in zip(
  ...
  current_images = np.array(
      list(map(lambda x: load_image(x), current_image_paths)))

  good_index = np.array(
      list(map(lambda x: x is not None, current_images)))
  ...
  for vis, ori, ori_path, l_name in zip(
          classmap_vis, current_images,
          current_image_paths, current_label_names):
    print(l_name)
    plt.imshow(ori)
    ...
    vis_path_ori = '../results/' + \
        ori_path.split('/')[-1].split('.')[0] + '.ori.jpg'
