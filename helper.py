...
def build_model(image_shape, n_classmap, n_labels, net_builder=_builder,
                is_prep=False, conv_args=(2, 1), checkpoint_path=None,
                optimizer=None, tensorboard_verbose=0, add_l1=True,
                max_checkpoints=None, tensorboard_dir='/tmp/tflearn_logs/'):
    ...
    conv_layer = tflearn.conv_2d(
        net, filter_size=conv_args[0], strides=conv_args[1],
        nb_filter=n_classmap)
    ...
    with tf.variable_scope('classmap'):
        label_w = tf.gather(tf.transpose(
            gap_w), tf.argmax(label_layer, axis=1))
    ...
    output_layer = tflearn.regression(net,
                                      label_layer,
                                      optimizer=optimizer,
    ...
    def infer_classmap(images, labels):
        ...
        classmap_answers = session.run(classmap_layer, feed_dict={
            ...
            label_layer: tf.one_hot(
                ...
                depth=n_labels).eval(session=session)})
